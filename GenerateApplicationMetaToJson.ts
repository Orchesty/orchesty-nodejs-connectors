    ...
    readdirSync(`${source}/${folderName}`, { withFileTypes: true })
        .forEach((item) => {
            if (item.isFile() && item.name.endsWith('Application.ts') && !(/A[A-Z].*/).exec(item.name)) {
                ...
                const reflectedClass: AApplication = Reflect.construct(classDefault, []);

                const fileMetadata: IFileMetadata = {
                    name: reflectedClass.getName(),
                    publicName: reflectedClass.getPublicName(),
                };
                ...
                fileMetadata.logo = reflectedClass.getLogo() ?? undefined;
