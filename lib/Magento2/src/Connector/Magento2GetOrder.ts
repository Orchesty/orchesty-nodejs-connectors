...
export default class Magento2GetOrder extends AConnector {
    ...
    public async processAction(dto: ProcessDto<IInput>): Promise<ProcessDto<IOutput>> {
        ...
        const { orderNumber } = dto.getJsonData();
        ...
        const res = await this.getSender().send<IOrderJson>(requestDto, [200, 404]);
        const order = res.getJsonBody();
...
export type IOutput = IOrderJson;
