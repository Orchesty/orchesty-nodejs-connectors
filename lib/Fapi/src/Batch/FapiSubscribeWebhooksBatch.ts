...
export default class FapiSubscribeWebhooksBatch extends ABatchNode {
    ...
    public async processAction(dto: BatchProcessDto): Promise<BatchProcessDto> {
        ...
        const { event, topology, node, token } = unsubscribedWebhook;
        const requestDto = application.getRequestDto(
            ...
            'connections',
            {
                ...
                application: 'global-webhook',
                credentials: {
                    event,
                    url: TopologyRunner.getWebhookUrl(topology, node, token),
                },
                meta_data: {
                    description: 'Orchesty Webhook',
                },
