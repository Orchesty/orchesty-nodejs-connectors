...
import MailstepApplication, { NAME as APPLICATION_NAME } from '../MailstepApplication';

export const NAME = `${APPLICATION_NAME}-unsubscribe-webhooks-batch`;

export default class MailstepUnsubscribeWebhooksBatch extends ABatchNode {
    ...
    public async processAction(dto: BatchProcessDto): Promise<BatchProcessDto> {
        ...
        if (subscribedWebhook) {
            const requestDto = await application.getRequestDto(
                ...
                applicationInstall,
                HttpMethods.DELETE,
                `eshop-webhook/${subscribedWebhook.getWebhookId()}`,
            );
            ...
            if (![
                ...
                StatusCodes.GONE,
            ].includes(responseDto.getResponseCode())) {
