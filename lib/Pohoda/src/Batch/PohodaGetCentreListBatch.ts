...
export const NAME = `${APPLICATION_NAME}-get-centre-list-batch`;

export const LAST_RUN_KEY = 'centre';
...
export default class PohodaGetCentreListBatch extends APohodaListBatch<unknown, IOutput, Filter> {
    ...
    protected getItems(response: IResponse): IOutput[] {
        ...
        const { listCentre } = responsePackItem;
        checkErrorInResponse(listCentre);

        const { itemCentre } = listCentre;
        checkErrorInResponse(itemCentre);
    ...
    protected async getCustomListRequestAttributes(dto: BatchProcessDto): Promise<object> {
        return {
            ...
            '@_version': '1.1',
            '@_centreVersion': undefined,
            'itemData:limit': undefined,
            'itemData:requestCentre': undefined,
